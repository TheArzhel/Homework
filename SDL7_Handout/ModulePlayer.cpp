...
bool ModulePlayer::Start()
{
	...
	player_pos = App->collision->AddCollider({ position.x, position.y, 32, 14 }, COLLIDER_PLAYER, this);
...
update_status ModulePlayer::Update()
{
	...
	player_pos->SetPos(position.x, position.y);
...
{
	if (col1->type==COLLIDER_WALL || col2->type==COLLIDER_WALL)
	App->player->Disable();
	App->fade->FadeToBlack(App->scene_space, App->scene_intro , 0.3f);
}
